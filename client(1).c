...
static void do_something()
{
	...
	if (select(serverfd+1, &fdlist, NULL, NULL, NULL) < 0) {
		...
		if (fgets(buf, sizeof buf, stdin) == NULL)
		    exit(0);
		char buffer[80];
		strncpy(buffer, buf, strlen(lang_say));
		buffer[strlen(lang_say)]=0;
		if(strcmp(buffer, lang_say) == 0){
		    strncpy(buffer, buf+strlen(lang_say)+1, strlen(buf)-strlen(lang_say));
		    sprintf(buf,"say %s\n",buffer);
		    send_string(buf);
		}else{
		    docmd(explode(buf));
...
static void getsaid(int obj){
  char buffer[200];
  strncpy(buffer, buf+8, sizeof(buf)-8);
  buffer[8]=0;  
